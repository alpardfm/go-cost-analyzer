...
**Common cases to consider suppressing:**

| Pattern | When to suppress |
|---------|-----------------|
| CEG-004 | Connection creation in constructor/factory functions (`NewServer`, `SetupDB`) that run once at startup |
| CEG-006 | Logging in error paths, startup code, or CLI tools where throughput isn't a concern |
| CEG-003 | Signal channels (`chan struct{}`, `chan error`) used for done/notify patterns |
| CEG-018 | Structs instantiated once (config, singleton) or generated by protobuf/ORM |
